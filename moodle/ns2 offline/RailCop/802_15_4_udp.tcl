...
set num_cross_flow [lindex $argv 2]		; #along row
...
set tr /home/mritunjoy/ubuntu/ns2\ programs/raw_data/multi_radio_802_15_4_random.tr
...
set tracefd [open /home/mritunjoy/ubuntu/ns2\ programs/raw_data/multi_radio_802_15_4_random.tr "w"]
...
while {$i < [expr ($num_row*$num_col)] } {
    $ns_ at $i "$node_($i) setdest [expr $x_dim*rand()] [expr $y_dim*rand()] $node_speed"
...
#CHNG
for {set i 1} {$i <= $num_parallel_flow } {incr i} {
	#set udp_node $i
	...
	#$ns_ attach-agent $node_($udp_node) $udp_($i)
  	...
  	set udp_node [expr ($i-1)*$num_col];
	set tempo [expr ($i-1)];
	set null_node [expr ((($num_col*$i))-1)];##CHNG
	$ns_ attach-agent $node_($udp_node) $udp_($tempo)
  	$ns_ attach-agent $node_($null_node) $null_($tempo)
...
for {set i 0} {$i < $num_cross_flow } {incr i} {
	...
	#$ns_ attach-agent $node_($udp_node) $udp_($k)
  	...
  	set udp_node $i 
	set null_node [expr ((($num_row-1)*$num_col)+$i) ];#CHNG
	$ns_ attach-agent $node_($udp_node) $udp_($k)
  	$ns_ attach-agent $node_($null_node) $null_($k)
...
proc finish {} {
	puts "finishing"
	global ns_ tracefd  topofile nm
	#global ns_ topofile
	$ns_ flush-trace
	close $tracefd
	#close $namtrace
