...
int is_window_full()
{
    if(seq_next >= base_seq + N)
        return 1;
    else
        return 0;
}

struct pkt *Get_Next_Free_Packet()
{   
   // printf(".............Inside  Get_Next_Free_Packet...........\n");
    int current_index = 0;
    if(seq_next - base_seq >= N)
    {
    ...
    current_index = (packets_base + seq_next - base_seq) % N;
    //printf(".............outside  Get_Next_Free_Packet...........\n");
    return &(Packets[current_index]);
}
...
{   
    //printf(".............Inside  Get_Packets...........\n");
    int current_index = 0;
    if(seqnum < base_seq || seqnum >= seq_next)
    {
        printf("Seqnum is not within the window\n");
        return NULL;
    }

    current_index = (packets_base + seqnum - base_seq) % N;
    //printf(".............outside  Get_Packets...........\n");
    return &(Packets[current_index]);
}
...
{
    //printf(".............Inside  Free_Packets...........\n");
    int Counts;
    if(acknum < base_seq || acknum >= seq_next)
        return;

    packets_base += (acknum - base_seq + 1);
    packets_base = packets_base % N;
    Counts = acknum - base_seq + 1;
    base_seq = acknum + 1;
    ...
    while(Counts>0)
    {
        ...
        mbuffer.pop();
        Extra_BufSize--;
        printf("calling A_output to send extra buffer message\n");
        A_output(m);
        Counts--;
    }
    //printf(".............outside  Free_Packets...........\n");
}
...
void A_output(struct msg message)
{
    ...
    p = Get_Next_Free_Packet();
    if(p == NULL)
    {
        printf("The window is full already\n");
        return;
    }
    layer5toA++;
    for (int i=0; i<20; i++)
    ...
    if(base_seq == seq_next)
    {
        starttimer(0,time_out);
    }
    ++seq_next;
    printf("Send packet to layer3\n");
...
void A_input(struct pkt packet)
{
    ...
    printf("Receive ACK packet from layer3\n");
    ...
    if(packet.acknum < base_seq)
    {
        printf("Receive duplicate ACK\n");
        return;
    }
    else if(packet.acknum >= seq_next)
    {
        printf("receive ACK of future Packets\n");
        return;
    }
    ...
    ackedpacket++;
    printf("%d ACK packet process successfully accomplished\n",ackedpacket);
...
{
    printf("Time interrupt occur\n");
    for(int i = base_seq; i < seq_next; ++i)
    {
        struct pkt *p = Get_Packet(i );
        printf("Timeout! Send out the package again\n");
        tolayer3(0, *p);
        ++Atolayer3;
    }
...
void B_input(struct pkt packet)
{
    ...
    printf("Receive a packet from layer3\n");
    Btolayer3++;
    if(packet.checksum !=checksum(&packet))
    {
    ...
    if(packet.seqnum == seqnum_of_B)
    {
        ++seqnum_of_B;
        tolayer5(1, packet.payload);
        ++Btolayer5;
        printf("Send packet to layer5\n");
    }
    else if(packet.seqnum < seqnum_of_B)
    {
        printf("Duplicated packet detected\n");
    }
    else
    {
        printf("Disordered packet received\n");
    }
    if(seqnum_of_B - 1 >= 0)
    {
        packet.acknum = seqnum_of_B - 1;	// resend the latest ACK
        packet.checksum = checksum(&packet);
        tolayer3(1, packet);
        printf("Send ACK packet to layer3\n");
    }
    printf("....... Outside  B_input......\n");
